...
# ============================================================================
# ReefBlueSky-Kh - Script de Download do login.html
# ============================================================================
# Uso: ./download-html.sh
# Este script baixa o login.html mais recente do GitHub
# ============================================================================
...
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  ReefBlueSky-Kh - Download de login.html                  ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""

# Verificar se o arquivo login.html existe
if [ -f "login.html" ]; then
  echo -e "${YELLOW}[INFO]${NC} Arquivo login.html atual encontrado"
  echo -e "${YELLOW}[INFO]${NC} Criando backup: ${BACKUP_DIR}/login.html.${TIMESTAMP}.bak"
  cp login.html "${BACKUP_DIR}/login.html.${TIMESTAMP}.bak"
  echo -e "${GREEN}[✓]${NC} Backup criado com sucesso"
  echo ""
else
  echo -e "${YELLOW}[INFO]${NC} Nenhum login.html anterior encontrado (primeira instalação)"
  echo ""
fi

# Download do arquivo
echo -e "${BLUE}[DOWNLOAD]${NC} Baixando login.html do GitHub..."
echo -e "${BLUE}[URL]${NC} ${REPO_URL}"
echo ""

if curl -f -o login.html "$REPO_URL"; then
  ...
  ls -lh login.html
  echo ""
  wc -l login.html | awk '{print "  Linhas: " $1}'
  echo ""
  ...
  if [ -f "${BACKUP_DIR}/login.html.${TIMESTAMP}.bak" ]; then
    ...
    echo "     ${BACKUP_DIR}/login.html.${TIMESTAMP}.bak"
  fi
  exit 1
fi
