    ...
    for (const d of rows) {
      ...
      if (isOffline && !d.offline_alert_sent) {
        try {
          ...
          console.log('[ALERT] E-mail de offline enviado para', d.email, 'device', d.deviceId);
        } catch (err) {
          console.error('[ALERT] Erro ao enviar alerta offline para device', d.deviceId, err.message);
        }
      }
