...
// ============================================================================
// WiFiSetup: Onboarding WiFi + Servidor (ESP32)
// ============================================================================

/**
 * Módulo de onboarding de rede e credenciais do servidor para o KH (ESP32).
 *
 * O que faz:
 * - Na primeira inicialização, cria um Access Point (AP) próprio.
 * - Disponibiliza um portal captive com formulário HTML.
 * - Coleta: SSID/senha do WiFi da casa + usuário/senha do servidor RBS.
 * - Testa conexão WiFi e persiste tudo em SPIFFS (+ credenciais em NVS).
 * - Gerencia múltiplos SSIDs e reconexão automática em background.
 *
 * Fluxo típico:
 * - Primeira vez:
 *   1) Cria AP "ReefBlueSkyKH-Setup".
 *   2) Usuário conecta no AP e acessa http://192.168.4.1.
 *   3) Preenche WiFi + login do servidor.
 *   4) Dispositivo conecta, registra no backend e salva a configuração.
 *
 * - Próximas inicializações:
 *   1) Lê wifi_config.json do SPIFFS.
 *   2) Tenta conectar usando a lista de redes salvas.
 *   3) Se falhar em todas, reabre o AP de configuração.
 */
...
private:
    ...
    // [SEGURANÇA] Arquivo de configuração
