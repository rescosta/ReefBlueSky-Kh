...
bool registerDevice() {
  ...
  if (httpCode == 201 || httpCode == 200) {
    ...
    String devToken    = data["token"].as<String>();
    String devRefresh  = data["refreshToken"].as<String>();
    unsigned long expiresIn = data["expiresIn"] | 3600;

    deviceToken = devToken;
    Serial.printf("✓ Device registrado! Device token recebido (len=%d).\n",
                  deviceToken.length());
                  
    Serial.printf("[DEBUG] deviceToken ORIGINAL len=%d\n", deviceToken.length());
    Serial.println("[DEBUG] deviceToken ORIGINAL raw:");
    Serial.println(deviceToken);
              

    unsigned long nowSec = getCurrentEpochMs() / 1000ULL;
    unsigned long expiry = nowSec + expiresIn;

    cloudAuth.storeTokenSecurely(devToken, devRefresh, expiry);

    Serial.println("[DeviceRegister] Token salvo na NVS via CloudAuth::storeTokenSecurely");
...
bool initMultiDeviceAuth() {
  ...
  // 1) Primeiro tenta usar token salvo via CloudAuth
  if (cloudAuth.init()) {
    Serial.println("[MultiDeviceAuth] Já tenho deviceToken válido, NÃO vou chamar /auth/login nem /device/register.");
    printConfigMenu();
    return true;
  }

  // 2) Só se não houver token válido é que entra no fluxo de credenciais/login
  Serial.println("[MultiDeviceAuth] Nenhum deviceToken válido, seguindo fluxo de login/registro...");
